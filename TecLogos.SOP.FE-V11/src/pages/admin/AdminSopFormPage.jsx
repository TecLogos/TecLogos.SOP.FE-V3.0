...
export default function AdminSopFormPage() {
  ...
  const [initialFile, setInit] = useState('')
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      fd.append('CommentText', form.remark)
      ...
      toast.error(validationMessage || err.response?.data?.message || err.response?.data?.Message || 'Save failed')
    } finally {
      setSaving(false)
    }
  }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-500 text-sm mt-0.5">
                {isView
                  ? 'View standard operating procedure details'
                  : isEdit
                    ? 'Update standard operating procedure details'
                    : 'Upload and submit a new standard operating procedure'}
              </p>
            </div>
          </div>
        ...
        <div className="card-surface border border-gray-200 rounded-2xl shadow-sm p-6 space-y-4 max-w-2xl">
          <div>
            ...
            <input
              ...
              value={form.sopTitle}
              disabled={isView}
              onChange={(event) => setForm((prev) => ({ ...prev, sopTitle: event.target.value }))}
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Enter SOP comments..."
            />
          </div>
              ...
              >
                ...
                <span className="text-sm text-gray-500">
                  {file
                    ? file.name
                    : initialFile && isEdit
                      ? `Current: ${initialFile.split(/[\\/]/).pop()} (Click to replace)`
                      : 'Click to browse PDF'}
                </span>
              </div>
              <input
